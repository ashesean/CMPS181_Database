...
//// New ixm functions for Project 4

RC RelationManager::createIndex(const string &tableName, const string &attributeName)
{
    string fileName = tableName + "." + attributeName + INDEX_FILE_EXTENSION;
    FileHandle handle;
    
    // Create the index file
    if (_ix->createFile(fileName) != SUCCESS)
        return 1;
    
    // Open the newly created index file for entry insertions
    if (_ix->openFile(fileName, handle) != SUCCESS)
        return 2;
    
    // Scan to retrieve the proper records
    vector<string> scanAttributeNames;
    scanAttributeNames.push_back(attributeName);
    RM_ScanIterator scanner;
    
    if (scan(tableName, "", NO_OP, NULL, scanAttributeNames, scanner) != SUCCESS)
        return 3;
    ...
    // Get the correct attribute data
    Attribute attribute;
    attribute.name = attributeName;
    bool found = false;
    
    for (unsigned i = 0; i < attrs.size() && !found; i++)
    {
    ...
    // Insert each tuple into the new index file
    RID rid;
    void *key = malloc(PAGE_SIZE);
    
    while (scanner.getNextTuple(rid, key) != RBFM_EOF)
    {
        if (_ix->insertEntry(handle, attribute, key, rid) != SUCCESS)
            return 6;
    }
...
{
    return _ix->destroyFile(tableName + "." + attributeName + INDEX_FILE_EXTENSION);
}
                              ...
                              )
{
    ...
    // Do index level scan
    if (_ix->scan(handle, attribute, lowKey, highKey, lowKeyInclusive, highKeyInclusive, rm_IndexScanIterator) != SUCCESS)
        return 4;
...
RC RelationManager::deleteTable(const string &tableName)
{
    ...
    //// Project 4: Delete appropriate index files
    ...
    // Delete the files whose names match: tableName.attributeName.ix
    string fileName;
    for (int i = 0; i < attrs.size(); i++)
    {
        fileName = tableName + "." + attrs[i].name + INDEX_FILE_EXTENSION;
        _ix->destroyFile(fileName);
    }
...
RC RelationManager::insertTuple(const string &tableName, const void *data, RID &rid)
{
    ...
    //// Project 4: Insert tuple into respective index files (if they exist)
    ...
    {
        // If an index file for this particular attribute exists, open it and insert the entry
        fileExists = false;
        fileName = tableName + "." + recordDescriptor[i].name + INDEX_FILE_EXTENSION;
        if (_ix->openFile(fileName, handle) == SUCCESS)
            fileExists = true;
        ...
        if (recordDescriptor[i].type == TypeInt)
        {
            if (fileExists)
            {
                memcpy(key, (char*) data + offset, INT_SIZE);
                if (_ix->insertEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
                    return 3;
                _ix->closeFile(handle);
            }
            offset += INT_SIZE;
        }
        else if (recordDescriptor[i].type == TypeReal)
        {
            if (fileExists)
            {
                memcpy(key, (char*) data + offset, REAL_SIZE);
                if (_ix->insertEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
                    return 3;
                _ix->closeFile(handle);
            }
        ...
        else
        {
            ...
            if (fileExists)
            {
                memcpy(key, (char*) data + offset, stringLength + VARCHAR_LENGTH_SIZE);
                if (_ix->insertEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
                    return 3;
                _ix->closeFile(handle);
            }
...
RC RelationManager::deleteTuples(const string &tableName)
{
    ...
    //// Project 4: Clear the appropriate index files. Uses same brutal approach as rbfm's deleteRecords().
    ...
    {
        fileName = tableName + "." + attrs[i].name + INDEX_FILE_EXTENSION;
        if (_ix->destroyFile(fileName) == SUCCESS)
            if (_ix->createFile(fileName) != SUCCESS)
                return 4;
    }
...
RC RelationManager::deleteTuple(const string &tableName, const RID &rid)
{
    ...
    //// Project 4: Get tuple's data; for deletion from index files.
    void *data = malloc(PAGE_SIZE);
    if (_rbfm->readRecord(fileHandle, recordDescriptor, rid, data) != SUCCESS)
        return 4;
    ////
    ...
    //// Project 4: Delete tuple from respective index files (if they exist)
    ...
    for (int i = 0; i < recordDescriptor.size(); i++)
    {
        // If an index file for this particular attribute exists, open it and delete the entry
        fileExists = false;
        fileName = tableName + "." + recordDescriptor[i].name + INDEX_FILE_EXTENSION;
        if (_ix->openFile(fileName, handle) == SUCCESS)
            fileExists = true;
        ...
        if (recordDescriptor[i].type == TypeInt)
        {
            ...
            {
                memcpy(key, (char*) data + offset, INT_SIZE);
                if (_ix->deleteEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
                    return 3;
                _ix->closeFile(handle);
            }
            offset += INT_SIZE;
...
RC RelationManager::updateTuple(const string &tableName, const void *data, const RID &rid)
{
    ...
    if (_rbfm->readRecord(fileHandle, recordDescriptor, rid, oldData) != SUCCESS)
        return 4;
    ////
    ...
    //// Project 4: Update index files using brutal approach similar to deleteTuples()
    ...
    for (int i = 0; i < recordDescriptor.size(); i++)
    {
        // If an index file for this particular attribute exists, open it and update the entry
        fileExists = false;
        fileName = tableName + "." + recordDescriptor[i].name + INDEX_FILE_EXTENSION;
        if (_ix->openFile(fileName, handle) == SUCCESS)
            fileExists = true;
        ...
        if (recordDescriptor[i].type == TypeInt)
        {
            if (fileExists)
            {
                memcpy(oldKey, (char*) oldData + oldOffset, INT_SIZE);
                memcpy(key, (char*) data + offset, INT_SIZE);
                if (_ix->deleteEntry(handle, recordDescriptor[i], oldKey, rid) != SUCCESS)
                    return 3;
                if (_ix->insertEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
                    return 5;
                _ix->closeFile(handle);
        ...
        else if (recordDescriptor[i].type == TypeReal)
        {
            if (fileExists)
            {
                memcpy(oldKey, (char*) oldData + oldOffset, REAL_SIZE);
                memcpy(key, (char*) data + offset, REAL_SIZE);
                if (_ix->deleteEntry(handle, recordDescriptor[i], oldKey, rid) != SUCCESS)
                    return 3;
                if (_ix->insertEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
        ...
        {
            unsigned oldStringLength, stringLength;
            memcpy(&oldStringLength, (char*) oldData + oldOffset, VARCHAR_LENGTH_SIZE);
            memcpy(&stringLength, (char*) data + offset, VARCHAR_LENGTH_SIZE);
            
            if (fileExists)
            {
                memcpy(oldKey, (char*) oldData + oldOffset, oldStringLength + VARCHAR_LENGTH_SIZE);
                memcpy(key, (char*) data + offset, stringLength + VARCHAR_LENGTH_SIZE);
                if (_ix->deleteEntry(handle, recordDescriptor[i], oldKey, rid) != SUCCESS)
                    return 3;
                if (_ix->insertEntry(handle, recordDescriptor[i], key, rid) != SUCCESS)
            ...
            offset += stringLength + VARCHAR_LENGTH_SIZE;
            oldOffset += oldStringLength + VARCHAR_LENGTH_SIZE;
        }
    }
